...
(function () {
  ...
  window.Views.orders = {
    ...
    render() {
      ...
      <div class="g12 section-gap">
       ...
       <div class="card section-gap"><div class="card-head"><div><div class="card-title">Khung giờ đặt đơn</div><div class="card-sub">thứ × giờ · ${S.periodLabel(st.period).toLowerCase()}</div></div></div><div class="card-pad" style="overflow-x:auto">${heatHTML(st.period, plat)}</div></div>
  ...
  window.Views.customers = {
    ...
    render() {
      ...
      const geo = S.cityDistribution(st.period, st.platform);
